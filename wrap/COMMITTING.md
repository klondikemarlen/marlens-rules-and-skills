...
- ❌ `:construction: Nest workflow SharePoint routes.` — does not say where they are nested
- ✅ `:construction: Nest workflow SharePoint routes under workflow namespace to match workflow template routing.` — names the target namespace and comparison point
- ❌ `:recycle: Align workflow SharePoint web routes.` — does not say what they align with
- ✅ `:recycle: Align workflow SharePoint web routes with template routes.` — names the comparison point
...
See https://github.com/icefoganalytics/wrap/issues/309
```
...
- Use exact UI element names: **Add Workflow**, **Submit for Approval**
- Reference menu locations: "top right dropdown nav", "left sidebar nav"
- Use navigation arrows: **Administration** → **Workflows** → **Create**
- Explicit verification: "Verify success message: 'Workflow created!'"
